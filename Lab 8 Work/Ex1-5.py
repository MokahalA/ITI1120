...
def make_matrix_random(a,b, x, y):
     ...
     for i in range(a):
          row=[]
          for j in range(b):
               row.append(random.randint(x,y+1))
          m.append(row)
     return m

#test example
print(make_matrix_random(3,3,4,10))
...
def sum_above_diagonal(m):
     ...
     for i in range(len(m)):
          for j in range(len(m[i])):
               if i <= j:
                    diagonal_sum = diagonal_sum + m[i][j]
     return diagonal_sum
...
def max_over_all_even_cols(m):
     ...
     for i in range(len(m)):
          for j in range(len(m[i])+1):
               if j%2==0:
                    max_list.append(m[i][j])
     return max(max_list)

#test example
print(max_over_all_even_cols([[1,1,1,1,1,1,1],[1,10,3,20,12,6,0]]))
...
print(index_of_max_sum_row([[100,100], [-100,0], [200,30], [10000,2]]))
